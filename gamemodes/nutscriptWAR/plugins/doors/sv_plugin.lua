...
--[[local function toggleLock(door, state)
...
function PLUGIN:PlayerUseDoor(client, entity)
	local faction = nut.faction.indices
	local items = client:getChar():getInv():getItems()

	for _, v in pairs(items) do 
		if v.isKey == true then
			client:getChar():setData("key", v:getData("key"))
			client:ConCommand("say /anon ОУЕ228: ну чо? Данные персонажа, ключа: "..v:getData("key"))
		--[[else
			client:getChar():setData("key", 0)]]
		end
	end

	--[[if !client:getChar():getData("key") == 0 then
		for _, v in ipairs(faction) do 
			client:ConCommand("say /anon ОУЕ228: ну чо? Данные двери:"..v.desc.." Данные персонажа:"..client:getChar():getData("key"))
			if v.isLockRoom == true and client:getChar():getData("key") == v.door_data.id then
...
function PLUGIN:CanPlayerAccessDoor(client, door, access)
	...
	local x = ""..faction..""
	if (faction and client:Team() == faction) or (faction and client:getChar():getInv():hasItem(x)) then
		client:ConCommand("say Тест 1: "..x)
		return true
	end
