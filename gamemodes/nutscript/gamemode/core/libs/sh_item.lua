	...
	if (uniqueID) then
		ITEM = (isBaseItem and nut.item.base or nut.item.list)[uniqueID] or setmetatable({}, meta)
			...
			ITEM.functions.drop = ITEM.functions.drop or {
				...
				onRun = function(item)
					item.player:ConCommand("say /zlostitem " ..item.name)
					item:transfer()
					item.player:EmitSound("stalker_sound/inv_drop.wav")
					return false
				end,
			...
			ITEM.functions.take = ITEM.functions.take or {
				...
				onRun = function(item)
					...
					item.player:ConCommand("say /ztakeitem " ..item.name)
					--item.player:EmitSound("interface/inv_take_7.ogg")
					if (!status) then
