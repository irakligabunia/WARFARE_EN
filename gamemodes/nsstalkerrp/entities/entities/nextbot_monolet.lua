...
ENT.PrintName = "Monolith Soldier"
...
function ENT:OnKilled( dmginfo )
	...
	nut.item.newInv(0, "loot", function(inventory)
		if (IsValid(entity)) then
			...
			for i = 2, 5 do
				inventory:add(drops_t[math.random( 1, #drops_t )])
			end
			if (math.random(1,16) == 1) then
				inventory:add(weapon_options[self.Weapon:GetClass()].item, 1, { 
					durability = math.random(3, 12)
				})
			end
			entity:setInventory(inventory)
			print("set inventory of nextbot")
		end
	end)
