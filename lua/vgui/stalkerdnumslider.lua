...
function PANEL:Init()
	...
	self.TextArea = self:Add( "DTextEntry" ) --ввод текста
	...
	self.Slider = self:Add( "StalkerDSlider", self )
	...
	self.Label = vgui.Create ( "DLabel", self ) --хуета, которая зумит перед скролом, но после текста
	self.Label:Dock( LEFT )
	self.Label:SetMouseInputEnabled( true )

	self.Scratch = self.Label:Add( "DNumberScratch" ) --тоже самое
	...
	--[[function self.Scratch:Paint(w,h)
		local material = Material("daui/gorscrl.png") 
		surface.SetDrawColor(color_white) 
		surface.SetMaterial(material) 
		surface.DrawTexturedRect(0, 0, w, h) 
	end

	function self.TextArea:Paint(w,h)
		local material = Material("daui/gorscrl.png") 
		surface.SetDrawColor(color_white) 
		surface.SetMaterial(material) 
		surface.DrawTexturedRect(0, 0, w, h) 
	...
	function self:Paint(w,h)
		...
		--[[surface.SetDrawColor(Color( 211, 211, 211, 255))
...
derma.DefineControl( "StalkerDNumSlider", "Menu Option Line", table.Copy( PANEL ), "Panel" )
...
-- Compat
derma.DefineControl( "StalkerSlider", "Backwards Compatibility", PANEL, "Panel" )
